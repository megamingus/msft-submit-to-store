...
namespace StoreSubmission
{
    ...
    /// </summary>
    public class FlightSubmissionUpdate
    {
        ...
        [DebuggerStepThrough]
        public FlightSubmissionUpdate(ClientConfiguration c)
        {
            this.ClientConfig = c;
        }
        ...
        public void RunFlightSubmissionUpdate()
        {
            ...
            dynamic flights = client.Invoke<dynamic>(
            HttpMethod.Get,
            relativeUrl: string.Format(
                CultureInfo.InvariantCulture,
                IngestionClient.GetApplicationFlightsUrlTemplate,
            ...
            for (int i = 0; i < flights.value.Count; i++)
            {
                if (flights.value[i].friendlyName == flightId)
                    flightId = flights.value[i].flightId;
